...
// 112) Given an array of integers nums, calculate the pivot index of this array.
...
// 113) Given a string paragraph and a string array of the banned words banned, return the most frequent
...
function mostCommonWord(str, banned) {
  ...
  for (let i = 0; i < validWords.length; i++) {
    wordCount[validWords[i]] = (wordCount[validWords[i]] || 0) + 1;
    if (wordCount[validWords[i]] > maxCount) {
      maxCount = wordCount[validWords[i]];
      maxWord = validWords[i];
    }
  }
  return maxWord || -1;
}
...
// 115) Given a string s, find the first non-repeating character in it and return its index. If it does not exist, return -1.
// Input: s = "leetcode"
// Output: 0
// Explanation:
...
// 116) We are playing the Guess Game. The game is as follows:
...
// 119) You are given an m x n integer grid accounts where accounts[i][j] is the amount of
...
// 124) Write an algorithm to determine if a number n is happy.
